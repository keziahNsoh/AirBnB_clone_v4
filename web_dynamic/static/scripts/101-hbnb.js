$(document).ready(() => {
  ...
  $('#toggle-reviews').on('click', function() {
    ...
    if (status === 'show') {
      $.ajax({
        url: `${HOST}:5001/reviews`,
        method: 'GET',
        success: function(data) {
          $('#review-list').html(data.map(review => `
            <div class="review">
              <h3>${review.title}</h3>
              <p>${review.text}</p>
            </div>
          `).join(''));
          $('#toggle-reviews').text('hide').data('status', 'hide');
        }
      });
    } else {
      $('#review-list').empty();
      $('#toggle-reviews').text('show').data('status', 'show');
    }
  });
});
